...
cfg_to_rcfg() {
  ...
  for r_file in *.cfg ; do
    mv $r_file `d_msg  $r_file | sed 's/.cfg//'`.rcfg
  done
  cd ../applications
  for r_file in *.cfg ; do
     mv $r_file `d_msg  $r_file | sed 's/.cfg//'`.rcfg
...
if [ ! -e $HOME/bin/realm ]; then
    setlang
    d_msg  "`cat << end $appname $ver
$WELCOME

end`"
	    
    inst_lang=`d_msg l 'Select Language' "$SETLANG" en de` && \
      setlang
      make_des
...
$WELCOME
    ...
    d_msg  
    inst_lang=`d_msg l 'Select Language' "$SETLANG" en de` && \
    if d_msg  f 'Already Installed' "$instappname $UPOR"
      make_des
      d_msg  "25%" "$COPDES"
      make_install
      d_msg  "50%" "$COPSH"
